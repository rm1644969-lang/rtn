...
## Live Market Admin Preview
- Admin future-candle monitor uses full, readable period IDs and a compact mobile horizontal strip.
- In SIMULATED mode, the next-candle direction is period-seeded and synchronized with the member-side Live Market engine, preventing red/green display mismatches.
- In LIVE FEED mode, a next-candle direction is shown only when the server feed publishes a matching future-period signal; otherwise the monitor shows SYNC WAIT rather than inventing a signal.
- Manual AUTO / GREEN / RED controls are display-preview controls only and persist in the admin browser; they do not change the market result or settlement.

## Live Market sync note
- Admin Live Market monitor is read-only for actual outcomes.
- In simulated mode, admin and member use the same period-seeded direction.
- In live-feed mode, the admin monitor shows SYNC WAIT unless the provider supplies an authoritative next-candle signal; it does not invent or override a future market outcome.
- Local GREEN/RED controls, where present, are test-preview only and do not settle member trades.
